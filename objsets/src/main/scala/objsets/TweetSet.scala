...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("TweetSet is empty")
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = {
    TweetReader.allTweets.filter(tw => google.exists(twText => tw.text.contains(twText)));
  }
  lazy val appleTweets: TweetSet = {
    TweetReader.allTweets.filter(tw => apple.exists(twText => tw.text.contains(twText)));
  }
